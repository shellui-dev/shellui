                ...
                <tr class=""border-b border-border transition-colors"">
                    ...
                    @foreach (var column in Columns)
                    {
                        <th class=""h-12 px-4 text-left align-middle font-medium text-muted-foreground @(column.Sortable ? ""cursor-pointer hover:bg-accent/50"" : """")""
                            @onclick=""async () => { if (column.Sortable) await OnSortClicked(column); }"">
    ...
    @if (ShowPagination && (_filteredItems.Any() || (ServerSide && _serverTotalCount > 0)))
    {
        <div class=""flex flex-col gap-4 px-2 py-4 sm:flex-row sm:items-center sm:justify-between"">
            <div class=""flex-1 text-sm text-muted-foreground"">
                ...
                {
                    @($""Showing {_startIndex} - {_endIndex} of {(ServerSide ? _serverTotalCount : _filteredItems.Count())} results"")
                }
            </div>
...
@code {
    ...
    {
        if (ServerSide && OnDataRequest is not null)
        {
    ...
    private async Task LoadServerPageAsync()
    {
        ...
        var request = new DataTableRequest
        {
            ...
            Take = _pageSize,
            SortBy = sortColumn?.PropertyName,
            SortDirection = sortColumn?.SortDirection ?? SortDirection.None,
            SearchQuery = _filterText ?? """"
        };

        var response = await OnDataRequest!(request);

        _paginatedItems = response.Items;
        _serverTotalCount = response.TotalCount;
        _totalPages = Math.Max(1, (int)Math.Ceiling(_serverTotalCount / (double)_pageSize));
        _currentPage = Math.Min(_currentPage, _totalPages);
        _startIndex = _serverTotalCount == 0 ? 0 : (_currentPage - 1) * _pageSize + 1;
        _endIndex = Math.Min(_currentPage * _pageSize, _serverTotalCount);
        _filteredItems = _paginatedItems;
    ...
    private async Task OnSortClicked(DataTableColumn<TItem> column)
    {
