...
public static class DashboardLayout02Template
{
    public static ComponentMetadata Metadata => new()
    {
        Name = "dashboard-02",
        DisplayName = "Dashboard 02",
        Description = "Sidebar + content layout. Sticky header (breadcrumb bar stays fixed on scroll, shadcn sidebar-02/16 style)",
        Category = ComponentCategory.Layout,
        FilePath = "DashboardLayout02.razor",
        IsLayoutBlock = true,
        Dependencies = new List<string> { "sidebar", "breadcrumb", "separator", "theme-toggle", "app-sidebar" },
        Tags = new List<string> { "dashboard", "sidebar", "layout", "block", "sticky" }
    };
...
@* Dashboard layout — sidebar + content. Sticky header stays fixed on scroll. *@
<SidebarProvider>
    <AppSidebar />
    <SidebarInset>
